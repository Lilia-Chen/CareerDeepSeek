import type { DecisionThreshold, OpportunityInput, Rubric, RubricDimension, ScoredOpportunity, ScoreContribution } from '../types.js'

export function scoreOpportunity(opportunity: OpportunityInput, rubric: Rubric): ScoredOpportunity {
...
function findHardBlockers(opportunity: OpportunityInput, rubric: Rubric): string[] {
  const flags = new Set(opportunity.riskFlags ?? [])
  return rubric.hardBlockers.filter(blocker => flags.has(blocker))
}

function assertOpportunityShape(opportunity: OpportunityInput): void {
...
function assertDimensionValue(opportunityId: string, dimension: RubricDimension, value: unknown): asserts value is number {
