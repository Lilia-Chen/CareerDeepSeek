  ...
  return {
    ...
    async generateJson(request) {
      ...
      if (request.task === "plan_visual_action") {
        if (request.state.url.includes("search.example")) {
          return {
            type: "click",
...
test("LLM visual planner returns coordinate-grounded actions through the action space", async () => {
  ...
  assert.equal(model.calls[0].task, "plan_visual_action");
  assert.equal(model.calls[0].state.visibleTextIncluded, true);
});

test("LLM evidence extractor returns page observations without raw visible text", async () => {
  const model = createMockModel();
  ...
  assert.equal(observation.extracted.target.id, "synthetic-runtime-lab");
  assert.equal(observation.evidence.length, 2);
  assert.equal(JSON.stringify(observation).includes(targetState.visibleText), false);
});

test("runs an end-to-end LLM discovery workflow and writes a private review queue item", async () => {
