...
it('scores a borderline strong opportunity as strong_fit', async () => {
...
it('hard blockers force reject even when the numeric score is high', async () => {
  const rubric = await loadRubric()
  const opportunity = {
...
it('throws when a fixture misses a rubric dimension', async () => {
  const rubric = await loadRubric()
  const opportunity = {
    id: 'missing-score',
