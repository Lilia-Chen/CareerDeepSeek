...
test("validates bounded visible-browser collection sessions", () => {
...
test("rejects forbidden browser actions before workflow execution", () => {
...
test("classifies visible page observations and builds scored target review items", async () => {
  const session = validateCollectionSession(syntheticSession);
  ...
  const scoredTarget = scoreTarget(observation.extracted.target, rubric);
...
test("writes review queue items only under the private data directory", async () => {
